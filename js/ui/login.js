...
async function doGoogle(card) {
  ...
  try {
    ...
    console.warn(e);
    toast('No se pudo iniciar sesión. Revisa tu conexión.');
...
function showPairing(card) {
  ...
  card.append(
    ...
    bigOption('🕊️', 'Entrar sin pareja', 'Explora la app; puedes vincularte después', () => finish()),
    bigOption('💌', 'Invitar a tu pareja :>', 'Genera un código único para compartir', () => showInvite(card)),
    bigOption('🔑', 'Tengo un código', 'Tu pareja ya te invitó', () => showJoin(card)),
  );
}

function bigOption(emoji, title, sub, fn) {
  return el('button', { class: 'login-option', onclick: fn }, [
    el('div', { class: 'lo-emoji', text: emoji }),
...
async function showInvite(card) {
  card.innerHTML = '';
  card.append(el('div', { style: { fontSize: '3rem' }, text: '💌' }), el('h2', { class: 'login-title', style: { fontSize: '1.4rem' }, text: 'Tu código de pareja' }), el('div', { class: 'sk', style: { height: '64px', margin: '14px 0' } }));
  let code;
